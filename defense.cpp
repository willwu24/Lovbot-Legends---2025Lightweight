...
void defenseMain(){
  ...
  {
    if((getEyeAngle()<=40||getEyeAngle()>=320)&&getEyeValue()>350){
      if(firstBallFront==-1){
...
void whiteMove(int dir)                       // dir = 90 or 270 (robot frame)
{
  ...
  angleRatio = 1-(exp(2.0 * (1.0 - angleRatio)) - 1.0) / (exp(2.0) - 1.0);   // high slope near 0, flat near 1
  ...
  else
  {
    ...
    finalDefenseSpeed = constrain(finalDefenseSpeed,15,90);
